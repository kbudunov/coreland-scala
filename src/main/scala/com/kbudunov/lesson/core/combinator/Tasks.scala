package com.kbudunov.lesson.core.combinator
...
object Tasks extends App {
  ...
  def getFirstValue1: Option[Int] =
    firstValue.map(e => e.toInt) //заходим в опшн, изменяем тип значения но НЕвытаскиваем
  def getFirstValue2 =
    firstValue.map(e => e.toInt).getOrElse(1) //явно ВЫТАСКИВАЕМ ииз опшна
  ...
  def getThirdValue1 =
    thirdValue.flatten.map(e => e.toInt).foldLeft(0)((m: Int, n: Int) => m + n)
  ...
  def getForthValue2 =
    forthValue.map(l => l.flatMap(s => s.map(e => e.toInt)).sum)
  ...
  def sumAll1: Option[Int] =
    opList1.map(l => l.flatten.flatten.map(e => e.toInt).sum)
//  val t: Seq[String] = opList1.map(list => list.flatten).map(e => e.map{ case Some(value) => value }).getOrElse(Seq.empty)
  def tSum: Option[Int] = opList1.map(_.flatten.flatten.map(_.toInt).sum)
  ...
  val opList31 =
    List(Some(Some(1)), Some(None), Some(Some(3)), Some(Some(4)), None)
  val sumAll21 = opList31.flatten.map(_.sum) //List(Some) флатиться!!!

  val numbs = Some(List(1))
//  val r = numbs.flatten.sum //Some(List(1)) НЕ  флатиться!!!

  private val fivethValue = Some(Some(List(Some("1"), Some("2"), Some("3"))))
  def getFivethValue =
    fivethValue.flatten.map(
      l => l.flatten.map(e => e.toInt).filter(e => e % 2 == 0).sum
    )
  val rt = fivethValue.flatten.map(_.flatten.map(_.toInt).sum)
